...
contract RoyaltiesRegistry is IRoyaltiesProvider, OwnableUpgradeable {
  ...
  function setRoyaltiesType(
    ...
    require(
      royaltiesType > 0 && royaltiesType <= royaltiesTypesAmount,
      "wrong royaltiesType"
    );
    royaltiesProviders[token] =
      uint256(royaltiesProvider) +
      2**(256 - royaltiesType);
  }
  ...
  function getRoyaltiesRaribleV2(address token, uint256 tokenId)
    internal
    view
    returns (LibPart.Part[] memory)
  {
    try RoyaltiesV2(token).getRaribleV2Royalties(tokenId) returns (
  ...
  function getRoyaltiesRaribleV1(address token, uint256 tokenId)
    internal
    view
    returns (LibPart.Part[] memory)
  {
